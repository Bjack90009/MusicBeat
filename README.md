...
- PERFECT 连击会显示在剩余时间下方，不再触发额外节拍或延长对局
- 游戏开始后出现一个名为 PICK ME 的彩虹色移动特殊节拍，结算时隐藏；它不缩圈、不消失，每次命中令当前普通节拍累计总分增加 10%
- PICK ME 命中时显示 `lalala`；总分实时按 `普通节拍累计原始分 × (1 + PICK ME 命中次数 × 0.1)` 重算
- PICK ME 当前速度按 `本局初始速度 × (1 + PICK ME 有效点击次数 × 0.2)` 实时重算；PICK ME 每 100ms 最多接受一次点击
- PICK ME 尺寸 72px，本局初始速度在 90–180px/s 内随机；每次有效点击立即随机转向 60°–150°，平时也会随机转向，碰到操作区边缘时反弹
- 玩家成功提交上榜名称后会保存在当前浏览器本地，下次进入上榜确认时自动带出，仍可修改
- 操作区上方左侧以上下两层显示总得分与 PICK ME 加成，得分数字与右侧倒计时等大；右侧显示剩余时间与 PERFECT COMBO，本局记录面板仅保留命中统计
- 首次准备态中央显示“准备好了吗？”和黄色操作说明；一局结束后中央改为显示排行榜前三名，并在榜单下方保留同一条操作说明
...
- 各品质生成/命中数量、逐次点击的剩余时间比例、普通节拍与移动白拍的鼠标/触屏/笔输入数量
- 移动白拍命中次数（`special_hits`），便于独立分析其得分贡献
- 粗粒度设备类别、视口大小、语言和时区
